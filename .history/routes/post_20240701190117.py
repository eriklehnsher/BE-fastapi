...
@router.get("/posts/category/{categories_name}", response_model=List[PostInDB])
async def get_posts_by_category_name(categories_name: str):
    posts = []
    cursor = xalothongtin_posts.find({"categories_name": categories_name})
    # Sử dụng to_list() để lấy danh sách kết quả từ cursor
    posts_docs = await cursor.to_list(length=None)
