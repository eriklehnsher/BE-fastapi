from ast import Param, Constant
...
async def upload_image(email: str, file: UploadFile = File(...)):
    ...
    if file_url.split("/")[0] == ".":
        file_url = file_url[1:]
    return {"email":email, "status": 200, "detail": "uploaded image", "file_url": file_url}

# upload user-image-ID
@router.post("/img/imageID")
async def upload_image(file: UploadFile = File(...)):
    path = f"{settings.upload_path}/users/imageID"
    filename = file.filename
...
@router.put("/img/avatar/{email}")
async def update_avatar(email: str, file: UploadFile = File(...)):
    path = f"{settings.upload_path}/users/avatars/"
    filename = file.filename
    extension = filename.split(".")[len(filename.split(".")) - 1]
    token_name = str(uuid.uuid4()) + "." + extension
