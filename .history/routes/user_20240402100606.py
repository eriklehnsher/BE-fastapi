...
async def update_user(email: str, updated_data: dict):
    ...
    user_in_db = Users_db.find_one({"email": email})
    if user_in_db is None:
        raise HTTPException(
            status_code=404, detail=f"User with email {email} not found"
        )
    
    # Cập nhật các trường chỉ khi chúng được cung cấp trong dữ liệu cập nhật
    update_fields = {}
    ...
    if "address" in updated_data:
        update_fields["address"] = updated_data["address"]
    # Thực hiện cập nhật
    result = Users_db.update_one({"email": email}, {"$set": update_fields})
