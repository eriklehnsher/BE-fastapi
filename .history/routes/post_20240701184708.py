...
async def create_post(post: PostModel):
    ...
    # Example: Process categories and tags if needed
    categories = []
    for category_name in post_dict.pop("categories_name", []):
        category = await get_category_by_name(category_name)  # Example function to get category by name
        if category:
            categories.append(CategoryInDB(**category.dict()))
    
    # Example: Save categories in post_dict
    post_dict["categories"] = categories
    
    # Example: Save other fields such as tags, images, etc.
