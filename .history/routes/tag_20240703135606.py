...
async def read_tags(skip: int = 0, limit: int = 10):
    try:
        tags_cursor = xalothongtin_tags.find().skip(skip).limit(limit)
        tags = []
        for document in tags_cursor:
            document["id"] = str(document["_id"])
            del document["_id"]
            tags.append(TagInDB(**document))
        return tags
...
async def update_tag(tag_id: str, tag: TagInDB):
    ...
    if await xalothongtin_tags.find_one({"_id": ObjectId(tag_id)}):
        await xalothongtin_tags.update_one(
            {"_id": ObjectId(tag_id)}, {"$set": tag_dict}
        )
        tag.id = tag_id
        return tag
    raise HTTPException(status_code=404, detail="Tag not found")
