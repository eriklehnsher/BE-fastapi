...
async def update_user(email: str, updated_user: UserInDB):
    ...
    @router.delete("/user/{user_id}", response_model=UserInDB)
async def delete_user_by_id(user_id: str):
    deleted_user = await Users_db.find_one_and_delete({"_id": ObjectId(user_id)})
    if deleted_user:
        return deleted_user
    raise HTTPException(status_code=404, detail=f"User with ID {user_id} not found")

@router.delete("/user/email/{email}", response_model=UserInDB)
async def delete_user_by_email(email: str):
    deleted_user = await Users_db.find_one_and_delete({"email": email})
    if deleted_user:
        return deleted_user
