...
async def create_user(user: UserRegister = Body(...)):
    ...
    if find_user_in_db is not None:
        ...
        return {**created_user, "access_token": access_token}
...
async def logout():
    try:
        # Gửi yêu cầu đến API logout của ứng dụng khác
        logout_response = requests.post(settings.other_app_api_url + "/logout")

        # Kiểm tra phản hồi từ API logout
        if logout_response.status_code == 200:
            return {"message": "User logged out successfully"}
        else:
            raise HTTPException(
                status_code=logout_response.status_code, detail="Logout failed"
            )

    except requests.RequestException as e:
        raise HTTPException(
            status_code=500, detail=f"Error while logging out: {str(e)}"
        )
...
async def show_user(id: str):
    ...
    user = Users_db.find_one({"_id": object_id})
    if user is not None:
        return user
