...
from models.car import CarInDB, UpdateCarInDB, CarModel
...
async def create_car(car: CarModel = Body(...)):
    car = car.model_dump()
    new_car =  db["Cars"].insert_one(car)
    created_car =  db["Cars"].find_one({"_id": new_car.inserted_id})
    return created_car
...
async def update_id(id: str, car: UpdateCarInDB = Body(...)):
    ...
    if len(car) >= 1:
        update_result = await db["Cars"].update_one(
...
async def delete_car(id: str):
    ...
    if delete_result.deleted_count == 1:
        return JSONResponse(
            status_code=status.HTTP_200_OK, content={"message": "Deleted successfully"}
        )
