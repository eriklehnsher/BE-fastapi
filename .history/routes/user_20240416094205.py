...
async def update_user(id: str, user_update: UserUpdateRequest):
    ...
    # Lấy dữ liệu cập nhật từ UserUpdateRequest
    ...
    updated_data = {
        "$set": {
            ...
            "full_Name": user_update_data.get("full_Name"),
            "phone": user_update_data.get("phone"),
            "address": user_update_data.get("address"),
            "avatar": user_update_data.get("avatar"),
            "DriverLicenseID": user_update_data.get("DriverLicenseID"),
            "birthdate": user_update_data.get("birthdate"),
        }
    }

    # Thực hiện cập nhật trong MongoDB
    result = Users_db.update_one({"_id": ObjectId(id)}, updated_data)
    ...
    if result.matched_count == 1 and result.modified_count == 1:
        ...
        # Nếu không có hoặc không cập nhật thành công, ném ra HTTPException
        raise HTTPException(
