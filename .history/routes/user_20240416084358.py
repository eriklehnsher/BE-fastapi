...
@router.patch("/user/update/{id}", response_model=UserInDB)
async def update_user(id: str, updated_user: UserUpdate = Body(...)):
    stored_user = Users_db.find_one({"_id": ObjectId(id)})
    if stored_user is None:
        raise HTTPException(status_code=404, detail=f"User with id {id} not found")
    stored_user.update(updated_user.model_dump(exclude_unset=True))
    stored_user = jsonable_encoder(stored_user)
    Users_db.update_one({"_id": ObjectId(id)}, {"$set": stored_user})
    return stored_user
